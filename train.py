...
def make_val_opt(opt):
    ...
    val_opt.num_threads = 4   # test code only supports num_threads = 1
    val_opt.batch_size = 8   # test code only supports batch_size = 1
...
def print_current_acc(log_name, epoch, score):
    ...
    Parameters:
    """
    message = '%s|(epoch: %d) ' % (log_name.split('/')[-2:],epoch)
...
def make_fcs_opt(opt):

    val_opt = copy.deepcopy(opt)
    val_opt.name = 'debug_train'
    val_opt.dataroot="../LEVIR-CD_cropped256/train/"
    val_opt.val_dataroot="../LEVIR-CD_cropped256/val/"
    val_opt.test_dataroot="../LEVIR-CD_cropped256/test/"
    val_opt.model='FCS'
# SA_mode=PAM
    val_opt.batch_size=64
    val_opt.arch='FCSD'
    val_opt.num_threads=4
    val_opt.save_epoch_freq=1
    val_opt.angle=15
    val_opt.preprocess = 'blur_rotate_transpose_hsvshift_noise_flip'
    val_opt.gpu_ids=[0]
    return val_opt
...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
       ...
       # miou_current = val(opt, model)
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            if total_iters % opt.print_freq == 0:   # print training losses and save logging information to the disk
                ...
                visualizer.print_current_losses(opt.model, epoch, epoch_iter, losses, t_comp, t_data)
                if opt.display_id > 0:
                    visualizer.plot_current_losses(epoch, float(epoch_iter) / dataset_size, losses)

            if total_iters % opt.save_latest_freq == 0:   # cache our latest model every <save_latest_freq> iterations
                print('saving the latest model (epoch %d, total_iters %d)' % (epoch, total_iters))
                save_suffix = 'iter_%d' % total_iters if opt.save_by_iter else '%s_latest'% opt.model
        ...
        if epoch % opt.save_epoch_freq == 0:              # cache our model every <save_epoch_freq> epochs
            ...
            model.save_networks(str(opt.model)+'_latest')
            miou_current = val(opt, model)
            if miou_current > miou_best:
                miou_best = miou_current
                epoch_best = epoch
                model.save_networks(str(opt.model)+'_'+str(epoch_best)+"_"+metric_name+'_'+'%0.5f'% miou_best)
