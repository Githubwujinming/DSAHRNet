...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    for label, im_data in visuals.items():
        if 'comp' in label:
            ...
            scale = 255
            norm = True
        im = util.tensor2im(im_data,scale=scale,normalize=norm)
...
class Visualizer():
            ...
            if ncols > 0:        # show all the images in one visdom panel
                ...
                for label, image in visuals.items():
                    if 'comp' in label:
                        ...
                        scale = 255
                        norm = True
                    image_numpy = util.tensor2im(image,scale=scale,normalize=norm)
            ...
            for n in range(epoch, 0, -1):
                ...
                for label, image_numpy in visuals.items():
                    if 'comp' in label:
                        ...
                        scale = 255
                        norm = True
                    image_numpy = util.tensor2im(image_numpy,scale=scale,normalize=norm)
    ...
    def print_current_losses(self,model, epoch, iters, losses, t_comp, t_data):
        ...
        """
        message = '(model: %s, epoch: %d, iters: %d, time: %.3f, data: %.3f) ' % (model, epoch, iters, t_comp, t_data)
