...
from FCCDN.GenerateNet import GenerateNet
from FCCDN.loss import FCCDN_loss_BCD, FCCDN_loss_SCD
# baseline0 
class FCCDNModel(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        self.netF = GenerateNet().to(self.device)
        if self.isTrain:
            # define loss functions
            self.criterionF = FCCDN_loss_SCD
            # initialize optimizers; schedulers will be automatically created by function <BaseModel.setup>.
            self.optimizer_G = torch.optim.AdamW(itertools.chain(self.netF.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999),weight_decay=0.001)
    ...
    def forward(self):
        self.dist, self.p1, self.p2 = self.netF(self.A, self.B)
        ...
        self.p2 = F.interpolate(self.p2, size=self.A.shape[2:], mode='bilinear',align_corners=True)
    ...
    def backward(self):
        ...
        # print(self.weight)
        self.loss_f = self.criterionF([self.dist, self.p1, self.p2], [self.L_s,self.L_s])
