...
# from . import loss
from ChangeFormer.losses import change_former_loss
from ChangeFormer.ChangeFormerNet import ChangeFormerV6

class ChangeFormerModel(BaseModel):
    """
    ...
    def __init__(self, opt):
        ...
        self.n_class = 2
        self.netF = ChangeFormerV6().to(self.device)

        if self.isTrain:
            # define loss functions
            self.criterionF = change_former_loss
    ...
    def forward(self):
        ...
        self.out = self.netF(self.A, self.B)  # f(A)
        self.dist = self.out[-1]
    ...
    def backward(self):
        ...
        # print(self.weight)
        self.loss_f = self.criterionF(self.out, self.L_s)
