...
class DAHRNModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netF = CDNet(fuse=opt.fuse).to(self.device)
        if self.isTrain:
            # define loss functions
            self.criterionF = DCAN_loss(opt.criterion)
    ...
    def forward(self):
        self.y1,self.y2,self.dist = self.netF(self.A, self.B)
    ...
    def backward(self):
        ...
        # print(self.weight)
        if self.criterion_type != 'hybrid':
            self.loss_f = self.criterionF(self.dist, self.y1, self.y2, self.L_s, 4)
        else:
