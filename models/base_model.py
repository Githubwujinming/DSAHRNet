...
class BaseModel(ABC):
    ...
    def setup(self, opt):
        ...
        if self.istest or not self.isTrain or opt.continue_train:
            load_suffix = 'iter_%d' % opt.load_iter if opt.load_iter > 0 else '%s_%s' % (opt.model,opt.epoch)
            self.load_networks(load_suffix)
        # self.print_networks(opt.verbose)
        expr_dir = os.path.join(opt.checkpoints_dir, opt.name)
        file_name = os.path.join(expr_dir, 'opt.txt')
        # params_num = self.print_networks(False)
        # there a bug for MFPN when execing "get_complexity_info() and val at the same time, you have to exec them separately"
        # macs, params = self.get_model_complexity_info()
        # with open(file_name, "a") as opt_file:
        #     opt_file.write('\n[Network %s_%s] Total number of parameters : { MACs: %s , Params: %s}' % (opt.model, opt.arch, macs, params))  # save the message

    def compare(self):
        tp = (self.L * self.pred_L_show)
        fp = (self.pred_L_show - tp)
        fn = self.L - tp
        tp = tp.repeat(1,3,1,1).permute(0,2,3,1)
        fp = fp.repeat(1,3,1,1).permute(0,2,3,1)
        fn = fn.repeat(1,3,1,1).permute(0,2,3,1)
        comp = torch.zeros_like(tp)
        comp[tp[:,:,:,0]==1] = torch.tensor([255,255,255]).to(self.device) #white
        comp[fp[:,:,:,0]==1] = torch.tensor([220,20,60]).to(self.device) #red
        comp[fn[:,:,:,0]==1] = torch.tensor([0,0,205]).to(self.device) # blue
        self.comp = comp.permute(0,3,1,2)
    ...
    def get_model_complexity_info(self):
        ...
        model = self.netF.to(self.device)
        if self.opt.model == 'FCS':
            if self.opt.arch == 'FCEF':
                x = torch.cat((x1,x2),1)[None,:]
