...
class SNUNet_ECAM(nn.Module):
    ...
    def forward(self, xA, xB):
        ...
        out = torch.cat([x0_1, x0_2, x0_3, x0_4], 1)# (N, 128, 256, 256)

        intra = torch.sum(torch.stack((x0_1, x0_2, x0_3, x0_4)), dim=0)#(N, 32, 256, 256)
        ca1 = self.ca1(intra) # (N, 32, 1, 1)
        # ca1 = ca1.repeat(1, 4, 1, 1)# (N, 128, 1, 1)
        # ca2 = self.ca(out)# (N, 128, 1, 1)
        out = self.ca(out) * (out + ca1.repeat(1, 4, 1, 1)) # (N, 128, 1, 1) + (N, 128, 256, 256)会不会牵强， 括号内为啥不是乘号用通道注意力
...
# import sys
# sys.path.append('/data/Siam-NestedUNet/utils')
# from utils.losses import hybrid_loss
if __name__ == '__main__':
    img1 = torch.randn(1, 3, 256, 256)
    img2 = torch.randn(1, 3, 256, 256)
    dev = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    model = SNUNet_ECAM(3, 2).to('cpu')
    # pred = model(img1, img2)
    # target = torch.ones(1, 2, 256, 256)
    # # criterion = hybrid_loss
    # cd_loss = criterion(pred, target)
