...
class mc_dice_bce_loss(nn.Module):
    """multi-class"""
    def __init__(self, weight=1, do_sigmoid = True):
        super(mc_dice_bce_loss, self).__init__()
        self.ce_loss = nn.CrossEntropyLoss(torch.tensor(weight))
        self.dice = MultiClass_DiceLoss(torch.tensor(weight), do_sigmoid)
...
def FCCDN_loss_BCD(scores, labels):
    ...
    out1[labels[1]==1]=0 # out1 = out1 *(1-labels)  y1 output in unchanged area
    out2[labels[1]==1]=0 # out2 = out2 *(1-labels)  y2 output in unchanged area
    out3[labels[1]!=1]=0 # out3 = out3 * labels y1 output in changed area
    out4[labels[1]!=1]=0 # out4 = out4 * labels y2 output in changed area

    pred_seg_pre_tmp1 = torch.ones(out1.shape).cuda()
    pred_seg_pre_tmp1[out1<=0.5]=0#  P1 in unchanged region
    pred_seg_post_tmp1 = torch.ones(out2.shape).cuda()
    pred_seg_post_tmp1[out2<=0.5]=0 # P2 in unchanged region
    
    pred_seg_pre_tmp2 = torch.ones(scores[1].shape).cuda()
    pred_seg_pre_tmp2[out3<=0.5]=0 # P1 in changed region
    pred_seg_post_tmp2 = torch.ones(scores[2].shape).cuda()
    pred_seg_post_tmp2[out4<=0.5]=0 # P2 in changed region
...
def FCCDN_loss_SCD(scores, labels):
    ...
    aux_loss = 0.2 * criterion(scores[1], pred_seg_post_unchange.float())
    aux_loss += 0.2 * criterion(scores[2], pred_seg_pre_unchange.float())
    aux_loss += 0.2 * criterion(scores[1], labels[1])
    aux_loss += 0.2 * criterion(scores[2], labels[1])
    loss = loss_change + loss_aux
    return loss
